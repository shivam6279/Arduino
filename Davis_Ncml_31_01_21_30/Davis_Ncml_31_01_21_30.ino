...
volatile int counter_offset = DATA_UPLOAD_FREQUENCY * 60;
...
//  sensors.begin();
  ...
  if(DATA_UPLOAD_FREQUENCY == 60) {
    counter_offset -= (current_time.minutes * 60 + current_time.seconds +  16);
    if(counter_offset < 0)
      counter_offset += DATA_UPLOAD_FREQUENCY;
  }
...
void loop() {
  ...
  uint8_t number_of_fail_uploads = 0;
  uint8_t initial_sd_card_uploads = 0;
...
//  }
  ...
  while(1) {
    ...
    if(four_sec) {
      ...
      if(SERIAL_OUTPUT) {
        Serial.print("\nTime left: ");
        Serial.print(seconds_remaining / 60);
        Serial.print(':');
        Serial.write((seconds_remaining % 60) / 10 + '0');
        Serial.write((seconds_remaining % 60) % 10 + '0');
        Serial.println();
      }
...
//          ShowSerialData();
      ...
      if(temp_read) { 
        ...
        if(DATA_UPLOAD_FREQUENCY == 60) {
          rounded_time = current_time.RoundToHour();
        } else {
          rounded_time.minutes = (rounded_time.minutes + MINUTE_ROUND/2)/MINUTE_ROUND * MINUTE_ROUND; //Round to next x hour mark
      ...
      if(temp_upload) { 
        ...
        if(RTC_flag) {
          ...
          if(DATA_UPLOAD_FREQUENCY == 60) {
            counter_offset -= (current_time.minutes * 60 + current_time.seconds + 8);
            if(counter_offset < 0)
              counter_offset += DATA_UPLOAD_FREQUENCY;
            timer1_counter = 0;
          }
...
ISR(TIMER1_COMPA_vect) { 
  ...
  #endif
  timer1_counter++;
  seconds_remaining = counter_offset - timer1_counter*4;
  if(seconds_remaining <= 0) { 
    timer1_counter = 0;
    read_flag = true;
    upload_flag = true;
    counter_offset = DATA_UPLOAD_FREQUENCY*60;
  }
  four_sec = true;
