...
String backup_id = "243"; //Backup id in case there is no sd card
...
struct weather_data{
  float temp1, temp2, hum, solar_radiation, latitude, longitude, voltage, battery_voltage, amps;
  int rain, wind_speed;
  long int pressure;
};
...
bool SubmitHttpRequest(){
  ...
  #endif
  while(Serial1.available()) Serial1.read();
  str_len = 66 + String(id).length()+4 + String(w.temp1).length()+4 + String(w.temp2).length()+4 + String(w.hum).length()+3 + String(w.wind_speed).length()+3 + String(w.rain).length()+3 + String(w.pressure).length()+3 + String(w.amps).length()+3 + String(w.voltage).length()+3 + String(w.battery_voltage).length()+4 + String(signal_strength).length()+3;
